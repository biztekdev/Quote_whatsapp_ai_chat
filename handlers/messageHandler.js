    ...
    async sendMessageOnce(messageId, to, message, type = 'text') {
        try {
            ...
            const isAutoGenerated = messageId.includes('auto-generated-') || 
                                   ...
                                   messageId.includes('-voice-to-text');
            ...
            console.log(`📤 Sending response for message ${messageId}${isAutoGenerated ? ' (auto-generated)' : ''}`);
...
Please extract all entities from the user message, selecting products, materials and finishes ONLY from the provided lists.`;
    ...
    async findCategoryByName(categoryName) {
        try {
            ...
            // Common spelling variations (incorrect → correct)
            ...
            case 'dimension_input':
                ...
                if (conversationData.selectedProduct && conversationData.selectedProduct.dimensionFields) {
                    ...
                    if (providedCount < requiredCount) {
                        console.log('🔍 Staying in dimension_input - missing dimensions:', {
    ...
    async handleFinishSelection(messageText, from, conversationData) {
        try {
            ...
            if (lowerText === '' || ['finish', 'finishes', 'what finishes', 'show finishes', 'finish options'].some(keyword => lowerText === keyword)) {
                console.log('🎨 User asking for finish options or empty message, showing available finishes');
    ...
    async handleQuantityInput(messageText, from, conversationData) {
        try {
            ...
            if (!extractedQuantity) {
                try {
                    ...
                    if (aiResponse.success && aiResponse.data?.entities?.['quantity:quantity']) {
                        ...
                        console.log('✅ Extracted quantity using AI:', extractedQuantity);
                    }
                } catch (aiError) {
            ...
            if (extractedQuantity && extractedQuantity > 0) {
                ...
                console.log('📦 Quantity saved:', extractedQuantity);
                
                await this.whatsappService.sendMessage(
                    from,
                    `Perfect! ✅ I've noted your quantity: **${extractedQuantity.toLocaleString()}** pieces.\n\nMoving to the next step...`
                );
                ...
                console.log('❌ Could not extract valid quantity from:', messageText);
...
Would you like to:`;
    ...
    validateQuoteData(conversationData) {
        ...
        if (!conversationData.dimensions || conversationData.dimensions.length === 0) {
            ...
            if (missingDimensions.length > 0) {
                ...
                missingFields.push(`• Missing required dimensions: ${missingDimNames}`);
                isValid = false;
                
                console.log('🔍 validateQuoteData - Missing dimensions:', {
    ...
    async handleAudioMessage(message, from) {
        try {
            ...
            // Send brief transcription confirmation to user
            await this.whatsappService.sendMessage(
                from,
            ...
            console.log('🔄 Processing voice-to-text message through text handler:', {
                originalId: message.id,
                newId: textMessage.id,
